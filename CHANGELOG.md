# DeepScout Chess — Change Log

> **Note on Historical Entries:** Entries prior to the Tauri v2 refactor (including references to PySide6, `LucasChessGui.py`, `Debug.py`, and `Window*.py`) represent historical change logs from the pre-Tauri prototype and are preserved strictly for algorithmic and changelog history.
...
- **Mass Analysis Database Save Resolution Fix (`DBgames.py`):** Fixed issue where worker threads completed game analysis but failed to update SQLite by updating `save_game_recno` to safely resolve whether the passed identifier is a zero-indexed grid `recno` or an explicit database `ROWID`, adding `_modify_by_rowid()` for direct persistence. (Issue #B43)
- **Redo Prior Analysis Overwrite & Post-Analysis Stats Update (`WDB_Games.py`):** Resolved issue where Mass Analysis skipped games when "Redo any existing prior analysis (overwrite existing evaluation tags)" was checked by setting `mode="OVERWRITE"`, and fixed post-analysis automatic statistics update by replacing obsolete `tw_themes()` call with `tw_generate_statistics()`. (Issue #B42)
- **Live Stockfish Default & Strict Adjudication Cascade (`gui_integration.py`, `WindowAnalysisParam.py`, & `result_repair.py`):** Configured Live Stockfish Engine Evaluation as the default primary adjudication policy across the Data Fitness Wizard and Mass Analysis parameters. Removed `None (Leave un-adjudicated)` options to enforce 100% result coverage. Added an automatic fallback cascade (Termination -> Accuracy/ACPL -> Last Move Turn -> Live Stockfish) so games missing PGN tags or ACPL ratings are seamlessly resolved by Stockfish. (Issue #B41)
- **Opening Book Pickle Setting Reset (`WindowAnalysisParam.py`):** Fixed issue where `GMopenings` re-appeared in the opening book filter box from user's previously saved `param_analysis.pickle` settings file by ensuring selecting `--` (no book) explicitly resets `analysis_params.book_name = None` on disk. (Issue #B40)
- **Opening Book Skip Override Fix (`WindowAnalysisParam.py`):** Resolved issue where Mass Analysis skipped the first 15-17 opening moves of every game by changing the default opening book filter fallback from `GMopenings` to `--` (`None`). Engine evaluation now analyzes games starting from Move 1. (Issue #B39)
- **`tw_themes` Menu Crash & Annotator Tag Fix (`WDB_Games.py` & `RunAnalysisControl.py`):** Fixed `AttributeError: 'WGames' object has no attribute 'tw_themes'` by routing the Utilities menu `Generate Statistics` option to `tw_generate_statistics()`, and updated worker completion handlers to explicitly write the active engine's name (e.g. `Stockfish 18`) to the `Annotator` tag. (Issue #B38)
- **Tab Index Offset Dispatch Fix (`WindowAnalysisParam.py`):** Added safe `hasattr(w, 'isChecked')` validation and `try/except` guard to the `dispatch` tab listener to prevent `AttributeError` when new tabs shift tab indices. (Issue #B37)
- **Gold Mass Analysis Action & Adjudication Tab (`gui_integration.py`, `WDB_Games.py`, & `WindowAnalysisParam.py`):** Added a Gold `🏆 Run Mass Analysis (Gold Standard)` action button directly inside the Data Fitness & Adjudication Wizard dialog, and created a dedicated `Adjudication Policy` tab inside the Mass Analysis parameters window for configuring primary and secondary fallback adjudication policies. (Issue #B35)
- **PGN Import PLYCOUNT Header Tag Honor (`DBgames.py`):** Updated `insert_pgn()` to check for explicit `[PlyCount]` or `[PLYCOUNT]` PGN header tags during bulk import before falling back to variation string length counting, ensuring imported games display their true move count in the grid. (Issue #B34)
- **Worker Thread Crash & Incremental Persistence (`RunAnalysisControl.py` & `DBgames.py`):** Resolved worker thread crash (`IndexError: list index out of range` in `read_game_recno`) by adding safe `recno` vs `rowid` bounds checking in worker prefetch loops, added `save_game_recno` database handler, and forced immediate incremental `conexion.commit()` after every analyzed game so no evaluation progress is lost on interrupt or error. (Issue #B33)
- **Gold & Silver Button Styling & Pruning Report (`gui_integration.py` & `WDB_Games.py`):** Styled the Mass Analysis button in the database readiness dialog with a Gold theme (`#D4AF37`) and the Quick Tier 2 button with a Silver theme (`#C0C0C0`). Added an explicit Data Fitness Cleaning Report popup displaying the exact count of zero-move / empty game records pruned from SQLite. (Issue #B32)
- **Stockfish Engine Path Prompt & Tie-Break Draw Assignment (`result_repair.py`):** Added an interactive `QFileDialog` file chooser prompt when Stockfish's path cannot be located (e.g. user replaced engine executable), and configured equal ACPL/Accuracy scores to automatically adjudicate as a `1/2-1/2` Draw. (Issue #B31)
- **PlyCount Half-Turn & Grid Candidate Bounds Fix (`DBgames.py` & `WDB_Games.py`):** Updated `PlyCount` calculation to inspect `mov_negra` on the final move to correctly count odd ply totals for games ending on White's turn, and applied safe list slicing (`li_row_ids[:grid.reccount()]`) to prevent `IndexError` on grid refreshes. (Issue #B30)
- **Quick Tier 2 Stats `IndexError` Fix (`WDB_Games.py` & `DBgames.py`):** Added `read_game_rowid()` and `read_complete_rowid()` methods to `DBgames` and updated `tw_quick_tier2_stats()` to query directly by SQLite `rowid` rather than treating `rowid` as a 0-based grid `recno` array index. (Issue #B28)
- **Database `read_game_raw` `TypeError` Fix (`DBgames.py`):** Handled `bytes` vs `str` type contract mismatch for database `_DATA_` BLOBs to prevent `.startswith()` crash during massive analysis and stats generation. (Issue #B27)
- **AI Narration Code Review Refactor (`Code/AI/Adapters.py` & `WDB_Players.py`):** Applied Kimi K3 code review fixes: added `return` guard to `tw_ai_summary()` to prevent unselected player crashes, implemented explicit `urllib.error.HTTPError` body logging, added `is_configured()` API key check, and refactored choice parsing defensively. (GitHub Issue #20)
- **MCP Tool Usage Protocol Documentation:** Documented workflow protocol issue regarding direct native MCP tool invocation vs redundant wrapper code creation. (GitHub Issue #21)
- **Unified Generate Statistics Toolbar Button & Database Metrics Columns (`WDB_Games.py`, `gui_integration.py`, `WDB_Players.py`, `DBgames.py`, & `WDB_Perfomance.py`):** Unified "Data Fitness" and "Generate Statistics" into a single "Generate Statistics" toolbar button with an integrated 2-step workflow (Result Repair -> Mass Analysis or Quick Tier 2 Stats). Added DuckDB status badge on Games toolbar, auto-created `GLICKO2`, `ESTIMATED_ELO`, `OPENING_ACC`, `MIDDLEGAME_ACC`, `ENDGAME_ACC`, `ACPL`, `ACCURACY` data columns in SQLite grid view, populated Glicko-2 and Estimated Elo in the Performance Rating matrix, and cleaned up Players tab toolbar buttons. (GitHub Issue #19)
