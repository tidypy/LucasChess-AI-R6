...
from Code.QT import Colocacion, Columnas, Delegados, FormLayout, Grid, Iconos, LCDialog, QTDialogs, QTMessages
...
class WUsuarios(LCDialog.LCDialog):
    ...
    def borrar(self):
        ...
        if row > 0:
            ...
            carpeta = "%s/users/%d/" % (self.configuration.paths.folder_userdata(), usuario.number)
            if os.path.isdir(carpeta):
                if QTMessages.pregunta(self, _("Do you want to remove all data of this user?")):
...
def set_password(procesador):
    ...
    while True:
        ...
        if resultado:
            ...
            if error:
                QTMessages.message_error(procesador.main_window, error)
