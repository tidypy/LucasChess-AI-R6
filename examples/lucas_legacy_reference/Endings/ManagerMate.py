    ...
    def create(self, num_blocks_level, num_positions_block, li_positions, from_sq):
        for nblock in range(num_blocks_level):
            ...
            block.create(
                num_positions_block,
                li_positions,
                from_sq + nblock * num_positions_block,
            )
            self.li_blocks.append(block)
...
class ControlMate:
    ...
    def __init__(self, manager, mate):
        ...
        self.mate = mate
        self.file_path = Code.configuration.paths.file_mate(mate)
        self.li_levels = None
        self.last_level = 0
    ...
    def create(self, num_levels, num_blocks_level, num_positions_block):
        ...
        if self.mate == 1:
            ...
            li_pos.extend(
                get_positions_fns(
                    Code.path_resource(
                        ...
                        "Mate in one (derived from mate in two).fns",
                    ),
                    parte_sadier,
                )
            )
            ...
            li_pos = get_positions_fns(
                Code.path_resource("Trainings", "Checkmates in GM games", "Mate in %d.fns" % self.mate),
                total_positions,
            )
