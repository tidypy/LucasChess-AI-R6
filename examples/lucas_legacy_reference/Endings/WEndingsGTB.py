...
from Code.Endings import EndingsGTB, LibChess
...
class WEndingsGTB(LCDialog.LCDialog):
    ...
    def __init__(self, wparent):
        ...
        self.color_done = ScreenUtils.qt_color_rgb(213, 233, 250)
        ...
        if position is not None:
            ...
            if mt is None or len(position.get_exmoves()) == 0:
                QTMessages.message_error(
                    self,
                    _("Invalid, this position is not evaluated by Gaviota Tablebases"),
                )
                return
            if mt < 0:
                QTMessages.message_error(self, _("Invalid, this position is lost"))
                return
    ...
    def restart(self):
        ...
        if (
            ...
            QTMessages.message_error(self, _("Invalid, this position is not evaluated by Gaviota Tablebases"))
            return
    ...
    def test_final(self):
        if self.game.is_finished():
            ...
            if ok:
                ...
                if not go_next:
                    QTMessages.message_accept(self, _("Done"), explanation=mensaje, delayed=True)
            else:
                QTMessages.message_error(
                    self,
    ...
    def utilities(self):
        ...
        submenusubmenu1.opcion(
            "remp_all_group",
            _("Delete all manually added positions in the current group"),
            Iconos.PuntoRojo(),
        )
        submenu.separador()
    ...
    def mensaje_import(self, num):
        if num:
            QTMessages.message(self, "%s: %d %s" % (_("Imported"), num, _("positions")))
        else:
            QTMessages.message_error(self, _("Nothing to import"))
...
def train_gtb(wparent):
    w = WEndingsGTB(wparent)
    if w.exec():
        if w.reinit:
            train_gtb(wparent)
