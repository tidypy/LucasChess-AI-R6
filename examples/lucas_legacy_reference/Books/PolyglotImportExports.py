...
class PolyglotExport:
    ...
    def export_create_polyglot(self, path_bin, uniform):
        ...
        bp = QTMessages.ProgressBarSimple(self.wpolyglot, _("Create book"), os.path.basename(path_bin), total)
...
class PolyglotImport:
    ...
    def menu_collisions(self):
        ...
        menu.opcion(
            "",
            _("What to do in case of collisions"),
            is_disabled=True,
            font_type=Controles.FontType(peso=700),
        )
        menu.separador()
    ...
    def import_polyglot(self):
        ...
        bp = QTMessages.ProgressBarSimple(self.wpolyglot, _("Import"), os.path.basename(path_bin), total)
    ...
    def import_db(self):
        path_db = QTDialogs.select_db(self.wpolyglot, self.configuration, True, False)
        if not path_db:
            return
    ...
    def merge(self, db, min_games, min_score, calc_weight, save_score, collisions):
        with QTMessages.one_moment_please(self.wpolyglot, _("Saving...")):
    ...
    def pasa_filtro(dic_act):
        ...
        if max_weight > 32767:
            ...
            for imove, (num, suma) in dic.items():
                dic_act[imove] = (
                    int(round(num / factor, 0)),
                    int(round(suma / factor, 0)),
                )
        return True
...
def create_bin_from_dbbig(owner, path_bin, db, min_games, min_score, calc_weight, save_score, uniform):
    ...
    with QTMessages.one_moment_please(owner, _("Saving...")):
        wpoly = FasterCode.PolyglotWriter(path_bin)
