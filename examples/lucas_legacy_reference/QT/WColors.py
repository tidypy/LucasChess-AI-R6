...
class WColors(LCDialog.LCDialog):
    def __init__(self, procesador):
        ...
        path_personal = self.configuration.paths.file_colors()
        dic = Util.ini_base2dic(path_personal, rfind_equal=True)
        self.dic_personal = {key: ScreenUtils.qt_color(value) for key, value in dic.items()}
    ...
    def grid_doble_click(self, grid, row, col):
        ...
        if col.key == "PERSONAL":
            ...
            qcolor_nuevo = QTDialogs.select_color(qcolor_previo)
            if qcolor_nuevo:
            ...
            for line in f:
                ...
                if line and not line.startswith("/"):
                    if current is None:
                        ...
                        self.dic_original[key_gen] = ScreenUtils.qt_color(color)
        return li
