...
class WTurnOnLights(LCDialog.LCDialog):
    ...
    def __init__(self, procesador, name, title, icono, folder, li_tam_blocks, one_line):
        ...
        self.colorTheme = ScreenUtils.qt_color("#F0F0F0")
    ...
    def right_button_theme(self, num_theme):
        ...
        menu.opcion(
            None,
            "%16s %15.02f  %s" % (_("Total time"), tt, time.strftime("%H:%M:%S", time.gmtime(tt))),
        )
        menu.lanza()
    ...
    def right_button_block(self, num_theme, num_block):
        ...
        if block.reinits:
            ...
            menu.opcion(
                None,
                plant % (_("Restarts"), tr, time.strftime("%H:%M:%S", time.gmtime(tr))),
            )
        ...
        menu.opcion(
            None,
            plant % (_("Total time"), tt, time.strftime("%H:%M:%S", time.gmtime(tt))),
        )
    ...
    def config(self):
        ...
        if self.one_line:
            menu.opcion(
                "change",
                _("Change options and create new training"),
                Iconos.TOLchange(),
            )
            menu.separador()
        ...
        if resp:
            if resp.startswith("t_"):
                self.tol.go_fast = ast.literal_eval(resp[2:])
                TurnOnLights.write_tol(self.tol)
            elif resp == "remove":
                self.rebuild()
            elif resp == "change":
    ...
    def cambiar_one_line(self):
        resp = config_oneline_tol(self, self.procesador)
        if resp:
            self.reinit = True
            self.save_video()
            self.accept()
...
def window_turn_on_ligths(procesador, name, title, icono, folder, li_tam_blocks, one_line):
    while True:
...
class WConfigOneLineTOL(LCDialog.LCDialog):
    def __init__(self, owner, procesador):
        ...
        lb_num_pos = Controles.LB2P(self, _("Number of positions"))
        li_blocks = [(str(x), x) for x in range(6, 60, 6)]
        self.cbNumPos = Controles.CB(self, li_blocks, self.tol.num_pos)

        lbtipo = Controles.LB2P(self, _("Mode"))
        li_tipos = [(_("Calculation mode"), True), (_("Memory mode"), False)]
        self.cbTipo = Controles.CB(self, li_tipos, self.tol.calculation_mode)

        lbfile = Controles.LB2P(self, _("File"))
        self.pbfile = Controles.PB(self, Code.relative_root(self.tol.fns), self.newfile, plano=False)
        lyfile = Colocacion.H().control(self.pbfile).relleno(1)
        ...
        # Colocamos ---------------------------------------------------------------
        ly = Colocacion.G()
        ly.controld(lb_num_pos, 1, 0).control(self.cbNumPos, 1, 1)
    ...
    def aceptar(self):
        ...
        if num_positions < self.cbNumPos.valor():
            QTMessages.message_error(
                self,
                _("This file has %d solved positions").replace("%d", str(num_positions)),
            )
            return
    ...
    def newfile(self):
        fns = TrainPositions.select_one_fns(self)
        if fns:
            ...
            if num_positions < self.cbNumPos.valor():
                ...
                return
            self.tol.fns = fns
            self.pbfile.setText(Code.relative_root(fns))


def config_oneline_tol(owner, procesador):
