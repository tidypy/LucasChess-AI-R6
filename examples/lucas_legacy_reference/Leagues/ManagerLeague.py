...
from Code.Engines import EngineManagerPlay, EngineResponse, EngineRun
from Code.Leagues import Leagues, WLeagues
...
class ManagerLeague(Manager.Manager):
    ...
    def base_inicio(self, league: Leagues.League, xmatch: Leagues.Match, division: int):
        ...
        engine = opponent_engine.thinker

        run_engine_params = EngineRun.RunEngineParams()
        run_engine_params.update_from_engine(engine)
        self.manager_rival = EngineManagerPlay.EngineManagerPlay(engine, run_engine_params)
        ...
        if bk:
            ...
            if self.league.book:
                ...
                if self.book.existe():
                    ...
                    self.book = None
        if self.book:
            self.manager_rival.set_book(self.book.path, self.book_rr, 5, self.book_depth)
        ...
        if self.timed:
            ...
            secs_extra = self.league.time_added_human * 60.0
