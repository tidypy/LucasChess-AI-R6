...
class WLeague(LCDialog.LCDialog):
    ...
    def __init__(self, w_parent, league):
        ...
        if league.current_num_season is None:
            with QTMessages.one_moment_please(w_parent):
                league.get_current_season()
        titulo = "%s - %s %d" % (
            league.name(),
            _("Season"),
            league.get_current_season() + 1,
        )
        icono = Iconos.League()
        extparam = "league1"
    ...
    def grid_doubleclick_header(self, grid, col):
        if grid == self.grid_matches:
            ...
            if order_next == 0:
                ...
                if col.key == "DIF_ELO":
                    li_panel.sort(
                        key=lambda x: x["ACT_ELO"] - x["INI_ELO"],
                        reverse=order_next == 2,
                    )
        ...
        if xmatch.result:
            ...
            if self.game_tmp:
                ...
                if result not in (RESULT_WIN_BLACK, RESULT_DRAW, RESULT_WIN_WHITE):
                    result = QTDialogs.get_result_game(self)
                    if result is None:
                        return
    ...
    def launch_worker(self):
        ...
        if resp:
            ...
            if self.pending_matches():
                ...
                QTMessages.message(self, _("There are no pending matches in the current matchday"))
    ...
    def export_to_database(self, dbf):
        ...
        pb = QTMessages.ProgressBarSimple(self, _("Generating the list of games to save"), "", total_games, 500)
        pb.mostrar()
        ...
        pb = QTMessages.ProgressBarSimple(self, _("Save all games to a database"), "", len(li_games), 500)
        pb.mostrar()
