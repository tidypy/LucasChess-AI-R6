...
from Code.QT import Colocacion, Controles, Iconos, LCDialog, QTDialogs, QTMessages, ScreenUtils
...
class WindowTutor(LCDialog.LCDialog):
    def __init__(self, manager, tutor, with_rival, with_openings, is_white, has_hints):
        ...
        tb_analisis.new("", Iconos.AIChip(), self.launch_ai_tutor, tool_tip=_("Explain Tutor Suggestion with AI"))
        lytbtutor = Colocacion.H().relleno().otro(lytbtutor).relleno().control(tb_analisis)

        tb_user_ai = Controles.TBrutina(self, icon_size=16, with_text=False)
        tb_user_ai.new("", Iconos.AIChip(), self.launch_ai_user, tool_tip=_("Explain Your Move with AI"))
        lytbuser = Colocacion.H().relleno().otro(lytbuser).relleno().control(tb_user_ai)

        if with_rival:
            tb_rival_ai = Controles.TBrutina(self, icon_size=16, with_text=False)
            tb_rival_ai.new("", Iconos.AIChip(), self.launch_ai_rival, tool_tip=_("Explain Opponent Prediction with AI"))
            lytbRival = Colocacion.H().relleno().otro(lytbRival).relleno().control(tb_rival_ai)
        ...
        self.cbRM, self.lbRM = QTMessages.combobox_lb(self, li_rm, li_rm[0][1], _("Moves analyzed"))
        ...
        layout.controlc(self.lb_player, fu, cu).controlc(self.board_user, fu + 1, cu).otro(
            lytbuser, fu + 2, cu
        ).controlc(bt_libros, fu + 3, cu)
        if with_rival:
    ...
    def launch_analysis(self):
        ...
        Analysis.show_analysis(
            self.manager.manager_analyzer,
            move_tmp,
            self.manager.board.is_white_bottom,
            pos_user,
            main_window=self,
            must_save=False,
        )
    ...
    def consult_book(self):
        li_movs = self.manager.consult_books(True)
        if li_movs:
            self.respLibro = li_movs[-1]
            self.save_video()
            self.accept()

    def launch_ai_commentary_for_rm(self, rm, title: str):
        from Code.AI.TutorCommentary import TutorCommentary
        try:
            fen = self.tutor.move.position_before.fen() if self.tutor and hasattr(self.tutor, 'move') and self.tutor.move else ""
            eval_txt = "Evaluation update"
            if rm:
                if hasattr(rm, "texto_rival") and "Prediction" in title:
                    eval_txt = str(rm.texto_rival())
                elif hasattr(rm, "texto"):
                    eval_txt = str(rm.texto())
                elif hasattr(rm, "abbrev_text_base"):
                    eval_txt = str(rm.abbrev_text_base())
            
            pv_raw = ""
            if rm:
                if hasattr(rm, "get_pv"):
                    pv_raw = rm.get_pv()
                elif hasattr(rm, "pv"):
                    pv_raw = rm.pv
            ...
            if pv_raw:
                try:
                    p = Game.Game(first_position=self.tutor.move.position_before)
                    p.read_pv(pv_raw)
            ...
            tc.explain_position_async(fen, eval_txt, main_line, title=title)
        except Exception as e:
            QTMessages.message_error(self, f"Could not generate AI explanation: {str(e)}")

    def launch_ai_tutor(self):
        rm = self.tutor.mrm_tutor.li_rm[0] if self.tutor and self.tutor.mrm_tutor and self.tutor.mrm_tutor.li_rm else None
        self.launch_ai_commentary_for_rm(rm, _("Tutor Suggestion AI Explanation"))

    def launch_ai_user(self):
        rm = self.tutor.rm_user if self.tutor else None
        self.launch_ai_commentary_for_rm(rm, _("Your Move AI Explanation"))

    def launch_ai_rival(self):
        rm = self.tutor.rm_rival if self.tutor else None
        self.launch_ai_commentary_for_rm(rm, _("Opponent Prediction AI Explanation"))

    def launch_ai_commentary(self):
        self.launch_ai_tutor()
