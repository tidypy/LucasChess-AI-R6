...
class ManagerTactics(Manager.Manager):
    ...
    def set_toolbar(self, modo):
        if modo == "end":
            li_options = [TB_CLOSE, TB_CONFIG, TB_UTILITIES, TB_NEXT]
            if not self.tactic.reinforcement.is_activated():
                li_options.insert(1, TB_CHANGE)
        else:
            li_options = [TB_CLOSE, TB_ADVICE, TB_REINIT, TB_CONFIG]
            if modo == "init":
                if not self.tactic.reinforcement.is_activated():
                    li_options.insert(2, TB_CHANGE)
    ...
    def end_game(self):
        ...
        resp = WindowTactics.historical_consult(self.procesador.main_window, self.tactic, icono)
        if resp:
            if resp != "seguir":
                if resp != "auto":
                    if resp.startswith("copia"):
                        ...
                        ncopia = None
                    if not WindowTactics.edit1tactica(self.procesador.main_window, self.tactic, ncopia):
                        return
            ...
            self.procesador.manager = ManagerTactics(self.procesador)
            self.procesador.manager.start(self.tactic)
