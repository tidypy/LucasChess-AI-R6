...
class GREngine:
        ...
        if nlevel == 0:
            ...
            while True:
                if len(d_engines[nlevel]) > 0:
                    Util.randomize()
                    engine_name, depth, elo = random.choice(d_engines[nlevel])
                    break
                else:
                    nlevel += x
                    if nlevel > 6:
    ...
    def elos(self):
        ...
        def mas(engine_name, xdepth, xelo):
            xelo = int(xelo)
            if xelo < 1100:
                tp = 1
            elif xelo < 1200:
...
class ManagerRoutesPlay(ManagerRoutes):
    ...
    def play_rival(self):
        if self.is_opening:
            ...
            pv = None
            if self.book:
                pv = self.book.select_move_type(fen, BOOK_RANDOM_UNIFORM)
                if not pv:
                    self.book = None
            if not pv:
    ...
    def player_has_moved_dispatcher(self, from_sq, to_sq, promotion=""):
        ...
        if self.is_opening:
            ...
            if pv != op_pv:
                if self.must_win:
                    QTMessages.temporary_message(self.main_window, _("Wrong move"), 2)
    ...
    def line_ended(self):
        ...
        if siwin:
            ...
            if self.must_win:
                QTMessages.message_error(self.main_window, _("You must win to pass this step."))
...
class ManagerRoutesTactics(ManagerRoutes):
        ...
        if move_obj.movimiento() != move_sel.movimiento():
            for variation in move_obj.variations.li_variations:
                ...
                if move_obj_v.movimiento() == move_sel.movimiento():
                    ...
                    self.continue_human()
                    return False
            QTMessages.temporary_message(self.main_window, _("Wrong move"), 0.8, physical_pos=TOP_RIGHT)
    ...
    def line_ended(self):
        ...
        if not self.route.go_fast:
            mensaje = "%s<br>%s" % (
                _("Done"),
                _("You have traveled %s") % Routes.km_mi(km, self.route.is_miles),
            )
            self.message_on_pgn(mensaje)
        self.human_is_playing = False
        self.state = ST_ENDGAME
