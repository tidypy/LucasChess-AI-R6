...
from Code.QT import Colocacion, Controles, Iconos, LCDialog, QTDialogs, QTUtils
        ...
        if li_moves:
            ...
            for n, mov in enumerate(li_moves):
                item = QtWidgets.QTreeWidgetItem(
                    father,
                    [mov.pgn, mov.porcentaje, mov.absoluto, f"{int(mov.absoluto):07}"],
                )
...
class WMoves(QtWidgets.QWidget):
    ...
    def rama(self):
        if self.tree.currentMov():
            QTUtils.send_key_widget(self.tree, QtCore.Qt.Key.Key_Plus, "+")
...
class InfoMove(QtWidgets.QWidget):
    ...
    def ponTituloLibro(self, titulo):
        self.lbTituloLibro.set_text(f"<h2>{titulo}</h2>")
...
class WindowArbolBook(LCDialog.LCDialog):
    def __init__(self, manager, si_en_vivo):
        ...
        # fens
        fen_activo = manager.fen_active()  # Posicion en el board
    ...
    def set_title(self, book):
        ...
        self.setWindowTitle(f"{_('Consult a book')} [{titulo}]")
    ...
    def menu_books(self):
        ...
        if n_books > 1:
            menu.separador()
            menu.opcion(("1", None), f"{_('Next book')} <F3>", Iconos.Buscar())
