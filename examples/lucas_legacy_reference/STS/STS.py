...
class Groups:
    def __init__(self, txt=None):
        if txt is None:
            with open(Code.path_resource("IntFiles", "STS.ini"), encoding="utf-8", errors="ignore") as f:
                txt = f.read()
...
class Work:
    ...
    def restore(self, dic):
        ...
        self.engine = Engines.engine_from_txt(dic["ENGINE"])
...
class Formula:
    ...
    def save_dic(self) -> dict:
        self.check_isdefault()
        return {"IS_DEFAULT": self.is_default, "X": self.x, "K": self.k}
...
class STS:
    ...
    def write_csv(self, fich):
        f = open(fich, "wt", encoding="utf-8", errors="ignore")
