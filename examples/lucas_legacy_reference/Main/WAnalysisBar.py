...
class AnalysisBar(QtWidgets.QWidget):
    def __init__(self, w_parent, board):
        ...
        layout = (
            Colocacion.V()
            .control(self.lb_value_up)
            .espacio(-6)
            .control(self.progressbar)
            .espacio(-6)
            .control(self.lb_value_down)
            .margen(0)
        )
    ...
    def get_config_play(self):
        if self.config_play is None:
            self.config_play = EngineRun.RunEngineParams()
            configuration = Code.configuration
            infinite = True
            if configuration.x_analyzer_mstime_ab:
                infinite = False
                self.config_play.fixed_ms = configuration.x_analyzer_mstime_ab
            if configuration.x_analyzer_depth_ab:
                infinite = False
                self.config_play.fixed_depth = configuration.x_analyzer_depth_ab
            if infinite:
    ...
    def configure(self):
        ...
        form.editbox(
            ...
            tipo=int,
            decimales=0,
            init_value=configuration.x_analyzer_depth_ab,
        )
