...
class WWashing(LCDialog.LCDialog):
    def __init__(self, procesador):
        ...
        self.dbwashing = Washing.DBWashing(Code.configuration)
        self.washing = self.dbwashing.washing
        eng = self.washing.last_engine(Code.configuration)
        ...
        if finished:
            ...
            html = html % (
                ...
                self.washing.total_engines(Code.configuration),
                _("Engine"),
                eng.name,
                _("Elo"),
                eng.elo,
    ...
    def file(self):
        ...
        if resp == "saveas":
            ...
            if resp.startswith("dbf_"):
                ...
                with QTMessages.WaitingMessage(self, _("Saving...")):
                    n = 0
                ...
                db.close()
                if n == 0:
                    QTMessages.message_bold(self, _("There are no games to be saved"))
                else:
                    QTMessages.message_bold(self, _X(_("Saved to %1"), database))
