...
class Opponent:
    ...
    def name(self):
        return self.thinker.name

    def elo(self):
        return self.thinker.elo
...
class Match:
    ...
    def list_non_existent_engines(self, league):
        ...
        li = []
        for opponent in (
            league.opponent_by_xid(self.xid_white),
            league.opponent_by_xid(self.xid_black),
        ):
    ...
    def add_results(self, dic, score_win, score_draw, score_lost):
        for xmatch in self.li_matches:
            ...
            dic[xid_white]["ACT_ELO"] = max(
                self.MIN_ELO,
                dic[xid_white]["ACT_ELO"] + Util.fide_elo(elo_w, elo_b, resn_w),
            )
            dic[xid_black]["ACT_ELO"] = max(
                self.MIN_ELO,
                dic[xid_black]["ACT_ELO"] + Util.fide_elo(elo_b, elo_w, resn_b),
            )
...
class League:
    ...
    def folder_work(self):
        return Util.opj(Code.configuration.paths.folder_leagues(), self.__name)
    ...
    def sort_list(self, modo):
        if modo == "division":
            self.li_opponents.sort(key=lambda x: x.initialdivision * 10000 + (10000 - x.thinker.elo))
        else:
            self.li_opponents.sort(key=lambda x: x.thinker.elo * 100 + (100 - x.initialdivision))
