...
from Code.Engines import EngineManagerPlay, EngineRun
...
class ManagerMotorAlbum:
    def __init__(self, manager, cromo):
        ...
        self.managerplay_irina = None
        self.managerplay_similar = None

        engine = self.manager.configuration.engines.search("irina")
        run_engine_params = EngineRun.RunEngineParams()
        run_engine_params.update(engine, 0, 1, 0, 1)
        self.managerplay_irina = EngineManagerPlay.EngineManagerPlay(engine, run_engine_params)

        self.opening = self.cromo.opening
        if self.opening:
            bookdef = Code.tbookPTZ
            self.managerplay_irina.set_book(bookdef, BOOK_RANDOM_UNIFORM, 5, 0)
    ...
    def cerrar(self):
        if self.managerplay_irina:
            self.managerplay_irina.close()
            self.managerplay_irina = None
        if self.managerplay_similar:
            self.managerplay_similar.close()
            self.managerplay_similar = None
    ...
    def juega_aleatorio(self, fen):
        self.managerplay_irina.set_option("Personality", "Random")
        return self.run_irina(fen)

    def juega_esquivo(self, fen):
        self.managerplay_irina.set_option("Personality", "Advance")
        return self.run_irina(fen)

    def juega_captura(self, fen):
        self.managerplay_irina.set_option("Personality", "Capture")
        return self.run_irina(fen)

    def juega_irina(self, fen):
        self.managerplay_irina.set_option("Personality", "Irina")
        return self.run_irina(fen)

    def run_irina(self, fen):
        rm = self.managerplay_irina.play(fen=fen)
        return rm

    def juega_similar(self, fen):
        if self.managerplay_similar is None:
            engine = self.manager.configuration.engines.search(self.cromo.engine)
            run_engine_params = EngineRun.RunEngineParams()
            run_engine_params.update(engine, 0, 5, 0, 10)
            self.managerplay_similar = EngineManagerPlay.EngineManagerPlay(engine, run_engine_params)
        self.managerplay_similar.play(fen=fen)
        mrm = self.managerplay_similar.mrm
        mrm.game = self.game
