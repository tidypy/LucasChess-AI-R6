...
class ManagerPlayHuman(Manager.Manager):
        ...
        if self.timed:
            ...
            if tc.time_is_consumed():
                self.game.set_termination(
                    TERMINATION_WIN_ON_TIME,
                    RESULT_WIN_BLACK if is_white else RESULT_WIN_WHITE,
                )
    ...
    def rendirse(self, with_question=True):
        ...
        if len(self.game) > 0:
            ...
            self.game.set_termination(
                TERMINATION_RESIGN,
                (RESULT_WIN_WHITE if not self.game.last_position.is_white else RESULT_WIN_BLACK),
            )
            self.autosave()
    ...
    def check_draw_player(self):
        if QTMessages.pregunta(self.main_window, _("Do both players agree to end the game with a draw?")):
