...
class ShowPGN(QtWidgets.QScrollArea):
    ...
    def right_click(self, lb_sender):
        ...
        if not text or 'href="' not in text:
            return
        if not self.selected_link or f'href="{self.selected_link}"' not in text:
        ...
        if resp is None:
            ...
            if QTMessages.pregunta(self, f"{_('Convert into the main line')}\n{_('Are you sure?')}"):
    ...
    def show_variations(self, work_move, selected_link):
        ...
        style_select = f"color:{Code.dic_colors['PGN_SELECT']}"
        ...
        def do_variation(variation_game: Game.Game, base_select):
            num_move = variation_game.first_num_move()
            li_pgn = []
            if variation_game.first_comment:
                li_pgn.append(f"{{{variation_game.first_comment}}}")
            ...
            for nvar_move, var_move in enumerate(variation_game.li_moves):
                ...
                xp = var_move.pgn_html_base(self.with_figurines) + var_move.resto(
                    with_variations=False, with_nag_symbols=True
                )

                link = f"{base_select}|{nvar_move}"
                style = style_select if link == selected_link else style_moves
                xp = f'<span style="{style}">{xp}</span>'
