...
class WBlqMove(QtWidgets.QWidget):
    ...
    def set_score(self, puntos):
        self.ms.set_text(f"{_('Score')}: {puntos}/100")
        self.ms.show()
        self.an.show()
...
class WPotenciaBase(LCDialog.LCDialog):
    def __init__(self, procesador):
        ...
        self.historico = PotenciaHistorico(self.configuration.paths.file_power())
        ...
        # self.pon_toolbar([self.finalize, self.empezar, self.repetir, self.configurar, self.borrar])
    ...
    def leeParametros(self):
        param = UtilSQL.DictSQL(self.configuration.paths.file_power(), tabla="parametros")
...
class WPotencia(LCDialog.LCDialog):
    ...
    def pensandoHastaMin(self):
        ...
        if dif <= 0:
            ...
            self.lbTiempo.set_text(
                _X(
                    _("%1 seconds remain to think moves before you can indicate them"),
                    str(dif),
                )
            )
    ...
    def comprobar(self):
        ...
        if not si_error:
            self.lbTiempo.set_text(f"<h2>{_('Result')}: {total_puntos} {_('pts')}</h2>")
            ...
            self.pon_toolbar([self.finalize])
