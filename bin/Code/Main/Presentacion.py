    ...
    def menu(self):
        ...
        for n, (fecha, pts) in enumerate(li_results, 1):
            if fecha == self.key:
                ...
                font_type = None
            txt = str(fecha)[:16]
            menu.opcion(None, f"{n:2d}. {txt:>16s} {pts:6d}", ico, font_type=font_type)
        ...
        if self.puntos_ultimo:
            menu.opcion(
                None,
                f"+{self.puntos_ultimo}".center(24),
                Iconos.PuntoNegro(),
                font_type=fbold,
            )
        if self.pendientes == 0:
            if not ok_en_lista:
                menu.opcion(
                    None,
                    ("%s: %d" % (_("Score"), self.puntos_totales)).center(30),
                    Iconos.Gris(),
                    font_type=fbold,
                )
            menu.separador()
        ...
        if from_sq + to_sq == self.result:  # No hay promotiones
            ...
            if self.intentos < self.max_intentos:
                QTMessages.temporary_message(
                    self.main_window,
                    str(self.max_intentos - self.intentos),
                    0.5,
                    puntos=20,
                    background="#ffd985",
                    with_image=False,
