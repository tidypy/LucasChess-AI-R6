...
class WForcingMoves(LCDialog.LCDialog):
    def __init__(self, owner):
        ...
        # Rotulo informacion
        self.lb_info_game = (
            Controles.LB(self, _("You can indicate the moves directly on the board."))
        ...
        self.ed_moves.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight)
    ...
    def pulsada_celda(self, celda):  # Incluida por compatibilidad del Board
        ...
        if self.level >= len(self.pvs):
            QTMessages.message_bold(self, _("You reached the end of the line!"))
            return
    ...
    def put_message(self, txt):
        ...
        QTMessages.message(self, txt, px=p.x(), py=p.y(), si_bold=True)

    def ask_question(self, txt):
        p0 = self.lb_info.pos()
        p = self.mapToGlobal(p0)
        return QTMessages.pregunta(self, txt, px=p.x(), py=p.y())
