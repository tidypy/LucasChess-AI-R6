...
class WTrainBMT(LCDialog.LCDialog):
    ...
    def __init__(self, owner, dbf):
        ...
        self.bmt_lista = Util.zip2var_change_import(dbf.leeOtroCampo(self.recnoActual, "BMT_LISTA"), li_replace)
        ...
        for bmt_lista in range(nposic):
            bt = Controles.PB(self, str(bmt_lista + 1), rutina=self.number).relative_width(44)
            bt.number = number
            number += 1
    ...
    def goto_pgn_base(self, row, column):
        if column == "NUMBER":
            if row == 0:
                ...
                row -= 1
        self.controlPGN.goto_row_iswhite(row, column == "WHITE")
    ...
    def original(self):
        ...
        for move in self.game.li_moves:
            ...
            if not move.position_before.is_white:
                row += 1
        self.goto_pgn_base(row, "WHITE" if si_w else "BLACK")
    ...
    def muestra(self, num):
        ...
        si_w = self.position.is_white
        self.goto_pgn_base(0, "WHITE" if si_w else "BLACK")
    ...
    def analize_position(self, row, key):
        ...
        move, is_white, si_ultimo, tam_lj, pos = self.get_movement(row, key)
        if move.is_mate:
            return

        Analysis.show_analysis(
            self.procesador.get_manager_analyzer(),
            move,
            is_white,
            pos,
            main_window=self,
