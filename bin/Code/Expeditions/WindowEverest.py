...
class WNewExpedition(LCDialog.LCDialog):
    def __init__(self, owner, file):
        ...
        # tolerance
        self.sbtolerance_min, lbtolerance_min = QTMessages.spinbox_lb(self, 20, 0, 99999, _("From"))
        self.sbtolerance_min.capture_changes(self.tolerance_changed)
        self.sbtolerance_max, lbtolerance_max = QTMessages.spinbox_lb(self, 1000, 0, 99999, _("To"))
        ...
        # repetitions
        self.sbtries_min, lbtries_min = QTMessages.spinbox_lb(self, 2, 1, 99999, _("From"))
        self.sbtries_min.capture_changes(self.tries_changed)
        self.sbtries_max, lbtries_max = QTMessages.spinbox_lb(self, 15, 1, 99999, _("To"))
    ...
    def mas(self):
        ...
        while True:
            ...
            form.apart_simple_np(
                ...
                % (
                    _("By example:"),
                    _("Number of games must be in range 12-500"),
            ...
            if resultado:
                ...
                if 12 <= nregs <= 500:
                    ...
                    QTMessages.message_error(
                        self,
                        "%s (%d)" % (_("Number of games must be in range 12-500"), nregs),
                    )
                    li_regs = None
            else:
                break
...
class WExpedition(LCDialog.LCDialog):
    def __init__(self, wowner, configuration, recno):
        ...
        self.color_negativo = ScreenUtils.qt_color_rgb(255, 0, 0)
        self.color_positivo = ScreenUtils.qt_color("#2b7d15")
...
class WEverest(LCDialog.LCDialog):
    ...
    def grid_dato(self, _grid, row, obj_column):
        ...
        if col in ("DATE_INIT", "DATE_END"):
            d = Util.stodext(v)
            v = Util.local_date_time(d) if d else ""

        elif col == "TIMES":
            li = ast.literal_eval(v)
