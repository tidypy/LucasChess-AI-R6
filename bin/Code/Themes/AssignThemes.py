...
class AssignThemes:
    def __init__(self):
        self.li_themes_run = [  # Th_AdvancedPawn.AdvancedPawn(),
            ...
            Th_Pin.Pin(),
            Th_UnderPromotion.UnderPromotion(),
            Th_Skewer.Skewer(),
            Th_DoubleCheck.DoubleCheck(),
        ]
        self.li_themes_mate_run = [
            Th_AnastasiaMate.AnastasiaMate(),
            Th_BodenMate.BodenMate(),
            Th_ArabianMate.ArabianMate(),
            Th_SmotheredMate.SmotheredMate(),
            Th_DovetailMate.DovetailMate(),
            Th_HookMate.HookMate(),
            Th_VukovicMate.VukovicMate(),
            Th_KillBoxMate.KillBoxMate(),
    ...
    def assign_game(self, game: Game.Game, with_tags: bool, reset: bool):
        ...
        if with_tags:
            if not reset:
                existing_tag = game.get_tag(TACTICTHEMES)
                if existing_tag:
                    existing_list = [t.strip() for t in existing_tag.split(",") if t.strip()]
                    li_themes = existing_list + li_themes
            if li_themes:
