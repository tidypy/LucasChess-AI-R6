...
class Transsiberian:
        ...
        for n, st in enumerate(line.li_st):
            ...
            li.append(
                ...
                % (
                    ...
                    st_from.name,
                    st.name,
                    km_mi(st.km - st_from.km, self._is_miles),
                    x2,
                )
            )
            st_from = st
    ...
    def next_task(self):
        ...
        if state == PLAYING:
            ...
            return (
                "<br>".join(litxt),
                Code.dic_colors["ROUTES_NEXTTASK_BACKGROUND_PLAYING"],
            )
        elif state == ENDING:
            return (
                _("You must solve an endgame puzzle"),
                Code.dic_colors["ROUTES_NEXTTASK_BACKGROUND_ENDINGS"],
            )
        else:
            return (
                _("You must solve tactics puzzles to advance."),
                Code.dic_colors["ROUTES_NEXTTASK_BACKGROUND_TACTICS"],
            )
