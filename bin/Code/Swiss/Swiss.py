...
class Opponent:
    ...
    def get_score(self, swiss):
        return (
            len(self.li_win) * swiss.score_win
            + len(self.li_draw) * swiss.score_draw
            + len(self.li_lost) * swiss.score_lost
            + self.byes * swiss.score_byes
        )

    def name(self):
        return self.thinker.name if self.thinker else ""

    def get_start_elo(self):
        return self.thinker.elo if self.thinker else 0
    ...
    def restore(self, dic):
        ...
        if t:
            if self.type == HUMAN:
                self.thinker = Human(None, None)
                self.thinker.restore(t)
            else:
                self.thinker = Engines.Engine()
                self.thinker.restore(t)
    ...
    def key_order(self, swiss):
        ...
        return (
            f"{score:04d}{score_sum_rival:05d}{len(self.li_win):03d}{len(self.li_draw):03d}"
    ...
    def reset(self, current_elo=None):
        ...
        self.li_draw = []
        self.li_lost = []
        self.current_elo = self.thinker.elo if current_elo is None else current_elo
...
class Match:
    ...
    def save(self):
        return {
            "XID": self.xid,
            "XID_WHITE": self.xid_white,
            "XID_BLACK": self.xid_black,
            "RESULT": self.result,
        }
...
class MatchsDay:
    ...
    def update_opponents(self, swiss):
        ...
        for xmatch in self.li_matches:
            ...
            op_white.current_elo = max(self.MIN_ELO, elo_w + Util.fide_elo(elo_w, elo_b, resn_w))
            op_black.current_elo = max(self.MIN_ELO, elo_b + Util.fide_elo(elo_b, elo_w, resn_b))
...
class Swiss:
    ...
    def list_engines(self):
        return [opponent.thinker for opponent in self.li_opponents if opponent.type == ENGINE]
    ...
    def set_engines(self, li_engines):
        ...
        for opponent in self.li_opponents:
            if opponent.type == ENGINE:
                xhash = opponent.thinker.xhash()
                if xhash in st_xhash:
                    li_opp.append(opponent)
                    st_ya.add(opponent.thinker.xhash())
            else:
                li_opp.append(opponent)
    ...
    def exist_engine(self, engine):
        for opponent in self.li_opponents:
            if opponent.type == ENGINE and opponent.thinker == engine:
                return True
        return False
