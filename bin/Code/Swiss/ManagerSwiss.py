...
from Code.Engines import EngineManagerPlay, EngineResponse, Engines
...
class ManagerSwiss(Manager.Manager):
    ...
    def base_inicio(self, swiss: Swiss.Swiss, xmatch: Swiss.Match):
        ...
        engine: Engines.Engine = opponent_engine.thinker
        self.manager_rival: EngineManagerPlay.EngineManagerPlay = self.procesador.create_manager_engine(
            engine, engine.max_time, engine.max_depth, engine.nodes
        )
        ...
        if bk:
            ...
            self.book.polyglot()
            self.book_rr = engine.book_rr
            self.book_depth = engine.book_max_plies
        else:
            if self.swiss.book:
        ...
        self.game.set_tag("Site", swiss.name())
        ...
        minutes, self.seconds_per_move = swiss.time_engine_human
    ...
    def restore_state(self, dic):
        swiss = Swiss.Swiss(dic["swiss_name"])
        xmatch = Swiss.Match("", "")
    ...
    def adjourn(self):
        if QTMessages.pregunta(self.main_window, _("Do you want to adjourn the game?")):
            ...
            label_menu = "%s: %s vs %s" % (
                self.swiss.name(),
                self.game.get_tag("WHITE"),
                self.game.get_tag("BLACK"),
            )
    ...
    def rendirse(self):
        ...
        self.game.set_termination(
            TERMINATION_RESIGN,
            RESULT_WIN_WHITE if self.is_engine_side_white else RESULT_WIN_BLACK,
        )
        self.crash_adjourn_end()
        self.set_end_game()
        self.autosave()
    ...
    def select_book_move(self, book, tipo, bdepth):
        if bdepth == 0 or len(self.game) < bdepth:
            ...
            pv = book.select_move_type(fen, tipo)
            if pv:
    ...
    def check_draw_resign(self):
        ...
        # Resign
        if self.swiss.resign > 0:
            si_resign = True
            for n, rm in enumerate(self.lirm_engine[-5:]):
                if rm.centipawns_abs() >= -100:
                    si_resign = False
                    break
                if int(-rm.centipawns_abs() * b) < self.swiss.resign:
                    si_resign = False
                    break
            if si_resign:
        ...
        # Draw
        if (self.swiss.draw_min_ply > 0) and (len(self.game) >= self.swiss.draw_min_ply):
            si_draw = True
