...
class WCountsCaptures(LCDialog.LCDialog):
    def __init__(self, procesador, is_captures):
        ...
        if is_captures:
            path = self.configuration.paths.file_captures()
            title = _("Captures and threats in a game")
            icon = Iconos.Captures()
            extconfig = "captures2"
        else:
            path = self.configuration.paths.file_counts()
            title = _("Count moves")
            icon = Iconos.Count()
            extconfig = "counts2"
    ...
    def new(self):
        ...
        menu.separador()
        if not QTDialogs.lista_db(Code.configuration, True).is_empty():
    ...
    def options(self):
        ...
        menu = QTDialogs.LCMenu(self)
        menu.opcion(True, _("Show all moves"), is_checked=showall)
        menu.separador()
        menu.opcion(False, _("Show last move"), is_checked=not showall)
        showall = menu.lanza()
        if showall is None:
            return
