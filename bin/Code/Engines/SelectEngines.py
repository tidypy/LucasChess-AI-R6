...
def read_uci_rodent(cm):
    with open(cm.path_uci, "rt", encoding="utf-8", errors="ignore") as f:
        for linea in f:
            linea = linea.strip()
...
def busca_engine(tipo, key, alias):
    if tipo is None:
        if key.startswith("*"):
            key = key[1:]
            tipo = ENG_EXTERNAL
...
def busca_engine_default(tipo, key, alias):
    resp = busca_engine(tipo, key, alias)
    return busca_engine(ENG_INTERNAL, Code.configuration.x_rival_inicial, None) if resp is None else resp


def gen_engines_irina():
    cmbase = Code.configuration.engines.search("irina")
    li = []
...
def gen_engines_rodent():
    cmbase = Code.configuration.engines.search("rodentii")
...
class SelectEngines:
    ...
    def __init__(self, owner):
        with QTMessages.one_moment_please(owner, _("Reading the list of engines")):
            ...
            self.liMotoresInternos = self.configuration.engines.list_name_internal()
            self.dic_engines_fixed_elo = self.configuration.engines.dic_fixed_elo()
            self.redo_external_engines()

            self.liIrina = gen_engines_irina()

            self.liElo = gen_engines_elo()

            self.dict_rodent = gen_engines_rodent()
    ...
    def menu(self, parent):
        ...
        for elo in li:
            ...
            lien = self.dic_engines_fixed_elo[elo]
    ...
    def list_all_engines(self):
        if self.li_engines is None:
            ...
            for elo in self.dic_engines_fixed_elo:
                lien = self.dic_engines_fixed_elo[elo]
                for cm in lien:
                    add_cm(cm, cm.name)
...
class WSelectEngines(LCDialog.LCDialog):
    def __init__(self, owner, list_all_engines, list_selected):
        ...
        tb = QTDialogs.LCTB(self, li_options, icon_size=24)
